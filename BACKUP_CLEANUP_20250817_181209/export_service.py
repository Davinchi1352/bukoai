...
class BookExportService:
    ...
    def export_book(self, book, format: ExportFormat, platform: ExportPlatform = ExportPlatform.STANDARD) -> Optional[str]:
        ...
        try:
            ...
            elif format == ExportFormat.MOBI:
                return self._export_mobi(book, config, cover_path, formatted_result)
            elif format == ExportFormat.AZW3:
                return self._export_azw3(book, config, cover_path, formatted_result)
    ...
    def _export_pdf(self, book, config: Dict[str, Any], cover_path: Optional[str], formatted_result: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """Export book as PDF with professional formatting and comprehensive features."""
        ...
        try:
            ...
            # Enhanced PDF configuration with professional settings
            pdf_config = self._enhance_pdf_config(config)
            
            # Create PDF document with enhanced page settings
            doc = SimpleDocTemplate(
                ...
                title=book.title,
                author=pdf_config.get('author_name', 'Buko AI'),
                subject=book.genre or 'Generated Book',
                creator='Buko AI Professional eBook Generator',
                keywords=self._extract_pdf_keywords(book)
            )
            
            # Create enhanced professional styles
            styles = self._create_enhanced_pdf_styles(pdf_config)
            story = []
            
            # Professional cover page with full formatting
            if pdf_config.get('include_cover_page', True):
                story.extend(self._create_professional_pdf_cover(book, cover_path, styles, pdf_config))
                story.append(PageBreak())
            
            # Title page with enhanced typography
            if pdf_config.get('include_title_page', True):
                story.extend(self._create_enhanced_title_page(book, styles, pdf_config))
                story.append(PageBreak())
            
            # Copyright page with professional formatting
            if pdf_config.get('include_copyright_page', True):
                story.extend(self._create_copyright_page(book, styles, pdf_config))
                story.append(PageBreak())
            
            # Dedication page if specified
            if pdf_config.get('include_dedication', False) and pdf_config.get('dedication_text'):
                story.extend(self._create_dedication_page(pdf_config['dedication_text'], styles))
                story.append(PageBreak())
            
            # Professional Table of Contents with bookmarks
            if pdf_config.get('include_table_of_contents', True):
                toc_data = self._create_enhanced_pdf_toc(book, styles, formatted_result)
                story.extend(toc_data)
                story.append(PageBreak())
            
            # Prologue if included
            if pdf_config.get('include_prologue', False) and pdf_config.get('prologue_text'):
                story.extend(self._create_prologue_section(pdf_config['prologue_text'], styles))
                story.append(PageBreak())
            
            # Main book content with professional formatting
            story.extend(self._process_enhanced_content_for_pdf(book, formatted_result, styles, pdf_config))
            
            # Epilogue if included
            if pdf_config.get('include_epilogue', False) and pdf_config.get('epilogue_text'):
                story.extend(self._create_epilogue_section(pdf_config['epilogue_text'], styles))
                story.append(PageBreak())
            
            # Acknowledgments if included
            if pdf_config.get('include_acknowledgments', False) and pdf_config.get('acknowledgments_text'):
                story.extend(self._create_acknowledgments_section(pdf_config['acknowledgments_text'], styles))
                story.append(PageBreak())
            
            # About author section
            if pdf_config.get('include_about_author', True):
                story.extend(self._create_about_author_section(book, styles, pdf_config))
                story.append(PageBreak())
            
            # Bibliography if specified
            if pdf_config.get('include_bibliography', False) and pdf_config.get('bibliography_entries'):
                story.extend(self._create_bibliography_section(pdf_config['bibliography_entries'], styles))
            
            # Build PDF with enhanced settings
            if pdf_config.get('include_bookmarks', True):
                doc.build(story, onFirstPage=self._pdf_first_page_template, 
                         onLaterPages=self._pdf_later_pages_template,
                         canvasmaker=self._enhanced_canvas_maker)
            else:
                doc.build(story)
            
            # Post-process PDF for additional features
            if pdf_config.get('optimize_for_print', False):
                self._optimize_pdf_for_print(file_path, pdf_config)
            
            if pdf_config.get('embed_fonts', False):
                self._embed_fonts_in_pdf(file_path, pdf_config)
            
            logger.info("pdf_exported_enhanced", 
                       book_id=book.id, 
                       file_path=file_path,
                       features_enabled=list(pdf_config.keys()))
            return file_path
    ...
    def _enhance_pdf_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Enhance PDF configuration with professional defaults and settings."""
        enhanced_config = config.copy()
        
        # PDF-specific professional enhancements
        professional_defaults = {
            ...
            'optimize_for_print': config.get('optimize_for_print', False),
            'embed_fonts': config.get('embed_fonts', False),
            'color_profile': config.get('color_profile', 'RGB'),
            'quality_level': config.get('quality_level', 85),
            'compression_level': config.get('compression_level', 6),
            'use_professional_typography': True,
            'include_page_numbers': True,
            'include_headers_footers': config.get('use_headers_footers', True),
            'chapter_start_odd_pages': config.get('start_chapters_on_odd_page', True),
            'include_drop_caps': config.get('use_drop_caps', False),
            'professional_margins': True
        }
        
        # Merge with existing config
        for key, value in professional_defaults.items():
            if key not in enhanced_config:
                enhanced_config[key] = value
        
        return enhanced_config
    
    def _create_enhanced_pdf_styles(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Create enhanced typography styles for professional PDF."""
        from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
        ...
        body_font = config.get('body_font_family', 'Times-Roman')
        heading_font = config.get('heading_font_family', 'Times-Bold')
        body_size = config.get('body_font_size', 11)
        
        # Enhanced title style
        custom_styles['EnhancedTitle'] = ParagraphStyle(
            'EnhancedTitle',
            parent=styles['Title'],
        ...
        custom_styles['ProfessionalBody'] = ParagraphStyle(
            'ProfessionalBody',
            ...
            spaceAfter=6,
            spaceBefore=6,
            alignment=TA_JUSTIFY if config.get('justify_text', True) else TA_LEFT,
            firstLineIndent=config.get('first_line_indent', 18)
        )
        
        # Chapter headings
        custom_styles['ChapterHeading'] = ParagraphStyle(
            'ChapterHeading',
            parent=styles['Heading1'],
            fontName=heading_font,
            fontSize=config.get('chapter_title_size', 18),
            spaceAfter=24,
            spaceBefore=36,
            alignment=TA_CENTER,
            textColor=colors.black,
        ...
        custom_styles['SubHeading'] = ParagraphStyle(
            ...
            fontName=heading_font,
            fontSize=config.get('heading_font_size', 14),
            spaceAfter=12,
            spaceBefore=18,
            alignment=TA_LEFT,
            keepWithNext=1
        ...
        # Copyright and metadata styles
        custom_styles['Copyright'] = ParagraphStyle(
            'Copyright',
            parent=styles['Normal'],
            fontName=body_font,
            fontSize=9,
    ...
    def _extract_pdf_keywords(self, book) -> str:
        """Extract keywords for PDF metadata."""
        keywords = []
        
        if book.genre:
            keywords.append(book.genre)
        
        if book.target_audience:
            keywords.append(book.target_audience)
        
        if hasattr(book, 'key_topics') and book.key_topics:
            keywords.extend(book.key_topics.split(',')[:5])  # First 5 topics
        
        keywords.append('Buko AI')
        keywords.append('eBook')
        
        return ', '.join(keywords[:10])  # Limit to 10 keywords
    
    def _create_professional_pdf_cover(self, book, cover_path: Optional[str], styles, config: Dict[str, Any]) -> List:
        """Create professional cover page for PDF."""
        ...
        if cover_path and os.path.exists(cover_path):
            ...
            try:
                ...
                cover_height = cover_width * 1.5  # Professional aspect ratio
                cover_story.append(RLImage(cover_path, width=cover_width, height=cover_height))
            except Exception as e:
                logger.warning("pdf_cover_image_failed", error=str(e))
                # Fallback to text cover
                cover_story.extend(self._create_text_cover(book, styles, config))
        else:
            # Create professional text-based cover
            cover_story.extend(self._create_text_cover(book, styles, config))
        
        return cover_story
    
    def _create_text_cover(self, book, styles, config: Dict[str, Any]) -> List:
        """Create professional text-based cover."""
        ...
        cover_elements.append(Paragraph(book.title, styles['EnhancedTitle']))
        cover_elements.append(Spacer(1, 0.5*inch))
        
        # Subtitle if available
        if config.get('subtitle'):
            cover_elements.append(Paragraph(config['subtitle'], styles['SubHeading']))
            cover_elements.append(Spacer(1, 0.5*inch))
        
        # Author
        author_name = config.get('author_name', 'Buko AI')
        cover_elements.append(Paragraph(f"Por {author_name}", styles['Metadata']))
        cover_elements.append(Spacer(1, 2*inch))
        
        # Publisher info at bottom
        if config.get('publisher'):
            cover_elements.append(Paragraph(config['publisher'], styles['Copyright']))
    ...
    def _export_mobi(self, book, config: Dict[str, Any], cover_path: Optional[str], formatted_result: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """Export book as MOBI with Kindle-optimized formatting."""
        try:
            # First generate EPUB as base
            epub_file = self._export_epub(book, config, cover_path, formatted_result)
            if not epub_file:
                logger.error("mobi_epub_base_failed", book_id=book.id)
                return None
            
            # Convert EPUB to MOBI using Calibre if available
            mobi_file = self._convert_to_kindle_format(epub_file, 'mobi', config)
            
            if mobi_file:
                logger.info("mobi_exported", book_id=book.id, file_path=mobi_file)
                return mobi_file
            else:
                logger.warning("mobi_conversion_failed", book_id=book.id)
                return epub_file  # Fallback to EPUB
            
        except Exception as e:
            logger.error("mobi_export_error", book_id=book.id, error=str(e))
            return None
    
    def _export_azw3(self, book, config: Dict[str, Any], cover_path: Optional[str], formatted_result: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """Export book as AZW3 with enhanced Kindle formatting."""
        try:
            # First generate EPUB as base with enhanced Kindle settings
            kindle_config = config.copy()
            kindle_config.update({
                'kindle_enhanced_typography': True,
                'include_x_ray_data': True,
                'optimize_for_kindle': True,
                'include_enhanced_typesetting': True,
                'kindle_format_version': 'KF8'
            })
            
            epub_file = self._export_epub(book, kindle_config, cover_path, formatted_result)
            if not epub_file:
                logger.error("azw3_epub_base_failed", book_id=book.id)
                return None
            
            # Convert EPUB to AZW3 using Calibre with enhanced settings
            azw3_file = self._convert_to_kindle_format(epub_file, 'azw3', kindle_config)
            
            if azw3_file:
                logger.info("azw3_exported", book_id=book.id, file_path=azw3_file)
                return azw3_file
            else:
                logger.warning("azw3_conversion_failed", book_id=book.id)
                return epub_file  # Fallback to EPUB
            
        except Exception as e:
            logger.error("azw3_export_error", book_id=book.id, error=str(e))
            return None
    
    def _convert_to_kindle_format(self, epub_file: str, target_format: str, config: Dict[str, Any]) -> Optional[str]:
        """
        Convert EPUB to Kindle format (MOBI/AZW3) using Calibre.
        Enhanced with quality settings and Kindle-specific optimizations.
        """
        try:
            import subprocess
            import shutil
            ...
            if not shutil.which('ebook-convert'):
                logger.warning("calibre_not_available", target_format=target_format)
                return None
            ...
            # Kindle-specific optimization options
            kindle_options = []
            
            # Output profile and format settings
            if target_format == 'mobi':
                kindle_options.extend([
                    '--output-profile', 'kindle',
                    '--mobi-file-type', 'old',  # MOBI 7 format for older Kindles
                    '--mobi-toc-at-start'
                ])
            else:  # azw3
                kindle_options.extend([
                    '--output-profile', 'kindle_dx',
                    '--mobi-file-type', 'new',  # KF8 format for newer Kindles
                    '--enable-heuristics',
                    '--mobi-toc-at-start'
                ])
            ...
            kindle_options.extend([
                ...
                '--dont-compress',  # Keep quality high
                '--max-toc-links', '0',  # No limit on TOC links
                '--no-inline-toc'  # Separate TOC
            ])
            
            # Typography and text settings
            if config.get('kindle_enhanced_typography', False):
                kindle_options.extend([
                    '--enable-heuristics',
                    '--smarten-punctuation',
                    '--unsmarten-punctuation'  # Let Kindle handle smart punctuation
                ])
            
            # Personal document settings
            if config.get('kindle_personal_doc', False):
                kindle_options.extend(['--personal-doc', '[PDOC]'])
            
            # Metadata settings
            if config.get('include_metadata', True):
                if 'author_name' in config and config['author_name']:
                    kindle_options.extend(['--authors', config['author_name']])
                
                if 'publisher' in config and config['publisher']:
                    kindle_options.extend(['--publisher', config['publisher']])
                
                if 'series_name' in config and config['series_name']:
                    kindle_options.extend(['--series', config['series_name']])
                    if 'series_index' in config:
                        kindle_options.extend(['--series-index', str(config['series_index'])])
            
            # Language settings
            if 'language' in config:
                kindle_options.extend(['--language', config['language']])
            
            # Advanced Kindle features for AZW3
            if target_format == 'azw3':
                # X-Ray support
                if config.get('include_x_ray_data', False):
                    kindle_options.append('--extract-to')
                    kindle_options.append(output_file + '_extracted')
                
                # Enhanced typesetting
                if config.get('include_enhanced_typesetting', False):
                    kindle_options.extend([
                        '--pretty-print',
                        '--expand-css'
                    ])
            ...
            result = subprocess.run(
                ...
                cwd=os.path.dirname(epub_file)
            )
            
            if result.returncode == 0:
                if os.path.exists(output_file):
                    logger.info("kindle_conversion_successful", 
                              source=epub_file,
                              target=output_file,
                              format=target_format)
                    return output_file
                else:
                    logger.error("kindle_conversion_output_missing", 
                               expected_file=output_file)
                    return None
            else:
                logger.error("kindle_conversion_failed",
                           format=target_format,
                           stderr=result.stderr[:500],  # Limit error message
                           returncode=result.returncode)
                return None
                
        except subprocess.TimeoutExpired:
            logger.error("kindle_conversion_timeout", format=target_format)
            return None
        except Exception as e:
            logger.error("kindle_conversion_error", 
                        format=target_format,
                        error=str(e))
            return None
...
# Make BookExportService available as ExportService for backward compatibility
ExportService = BookExportService
