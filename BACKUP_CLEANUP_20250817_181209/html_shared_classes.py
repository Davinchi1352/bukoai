"""
HTML Structure and Book Formatting Service
Procesa contenido HTML de Claude AI y lo estructura para formateo profesional de ebooks.

NOTA: A pesar del nombre del archivo, este servicio procesa contenido HTML (no markdown).
Claude AI genera HTML puro que este servicio estructura en elementos semánticos.
TODO: Renombrar archivo a html_structure_service.py en futura refactorización.
"""
...
class HTMLElement:
    ...
    def _build_attributes(self) -> str:
        ...
        for key, value in self.attributes.items():
            if key != "class":
                # 🔧 FIX: No escapar el atributo style para preservar CSS válido
                # Las propiedades CSS con quotes como font-feature-settings: "kern" 1
                # deben mantenerse intactas para funcionar correctamente
                if key == "style":
                    attrs.append(f'{key}="{str(value)}"')
                else:
...
class MarkdownToHTMLConverter:
    """
    Procesador de estructura HTML para libros.
    
    NOTA: A pesar del nombre, procesa contenido HTML generado por Claude AI.
    Convierte HTML plano en estructura semántica con elementos tipados.
    TODO: Renombrar a HTMLStructureParser en futura refactorización.
    """
    ...
    def convert(self, html_content: str, book_title: str = "", 
                author: str = "", language: str = "es") -> BookStructure:
        """
        Convierte contenido HTML de Claude AI a estructura de libro profesional.
        ...
        Returns:
            BookStructure con elementos HTML estructurados semánticamente
        """
        
        # Pre-procesar el contenido HTML (normalización)
        processed_content = self._preprocess_markdown(html_content)  # TODO: Renombrar método
        
        # Parsear elementos HTML a estructura semántica
        elements = self._parse_markdown_to_elements(processed_content)  # TODO: Renombrar método
    ...
    def _preprocess_markdown(self, content: str) -> str:
        """
        Pre-procesa el contenido HTML para normalización.
        
        NOTA: A pesar del nombre, procesa contenido HTML de Claude AI.
        TODO: Renombrar a _preprocess_html_content().
        """
    ...
    def _parse_markdown_to_elements(self, content: str) -> List[HTMLElement]:
        """
        Parsea el contenido HTML a elementos estructurados semánticamente.
        
        NOTA: A pesar del nombre, procesa contenido HTML de Claude AI.
        Combina parsing de HTML y markdown syntax por compatibilidad.
        TODO: Renombrar a _parse_html_to_elements() y optimizar para HTML puro.
        """
        elements = []
        lines = content.split('\n')
        i = 0
...
# NOTA: La función convert_markdown_to_content_html fue eliminada porque:
# 1. Claude AI genera contenido exclusivamente en HTML puro
# 2. Nunca se ejecutaba (condición imposible en approve_architecture)
# 3. El contenido siempre se guarda como HTML en ambos campos (content y content_html)